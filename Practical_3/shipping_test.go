// shipping_test.go - Tests for original version with updated function name
package shipping
...
func TestCalculateShippingFeeV1_EquivalencePartitioning(t *testing.T) {
	// Test P1: A weight that is too small (e.g., -5). We expect an error.
	_, err := CalculateShippingFeeV1(-5, "Domestic")
	if err == nil {
	...
	fee, err := CalculateShippingFeeV1(10, "Domestic")
	if err != nil {
	...
	_, err = CalculateShippingFeeV1(100, "International")
	if err == nil {
	...
	_, err = CalculateShippingFeeV1(20, "Local") // Using a valid weight
	if err == nil {
		t.Error("Test failed: Expected an error for an invalid zone, but got nil")
	}
}

func TestCalculateShippingFeeV1_BoundaryValueAnalysis(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			_, err := CalculateShippingFeeV1(tc.weight, tc.zone)
...
func TestCalculateShippingFeeV1_DecisionTable(t *testing.T) {
	testCases := []struct {
		name          string
		weight        float64
		zone          string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			fee, err := CalculateShippingFeeV1(tc.weight, tc.zone)
