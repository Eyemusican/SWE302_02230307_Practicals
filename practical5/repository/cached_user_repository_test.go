...
func TestMain(m *testing.M) {
	...
	testDB, err = sql.Open("postgres", pgConnStr)
	if err != nil {
...
func TestCachedGetByID(t *testing.T) {
	ctx := context.Background()
	repo := NewCachedUserRepository(testDB, testRedis)
...
func TestCachedCreate(t *testing.T) {
	...
	exists, _ := testRedis.Exists(ctx, cacheKey).Result()
	if exists != 1 {
...
func TestCacheExpiration(t *testing.T) {
	...
	cacheKey := fmt.Sprintf("user:%d", user.ID)
	ttl, err := testRedis.TTL(ctx, cacheKey).Result()
	if err != nil {
